// ==========================================
// 1. MOTEUR AUDIO (Web Audio API Synth)
// ==========================================
const AudioEngine = {
    ctx: null,
    muted: false,
...
const App = {
    ...
    updateCreditsDisplay() {
        document.getElementById('user-credits').innerText = Math.floor(this.credits);
    },
    ...
    runBootSequence() {
        ...
        document.getElementById('btn-enter').addEventListener('click', () => {
            AudioEngine.init(); // Initiale l'audio au premier clic humain (Requis par les navigateurs)
            AudioEngine.sfxClick();
...
// ==========================================
// 3. JEU : CYBER BLACKJACK
// ==========================================
const Blackjack = {
    deck: [], playerHand: [], dealerHand: [],
    bet: 100, playing: false,
    ...
    buildDeck() {
        ...
        // Shuffle algorithme Fisher-Yates
    ...
    updateBetDisplay() {
        document.getElementById('bj-current-bet').innerText = this.bet;
    },
    ...
    async deal() {
        if(App.credits < this.bet) {
            document.getElementById('bj-message').innerText = "FONDS INSUFFISANTS";
            return;
        }
        
        App.updateCredits(-this.bet); // Retire la mise
        AudioEngine.sfxClick();
        ...
        // Distribution animée asynchrone
        ...
        await this.addCardToHand(this.playerHand, 'bj-player-cards', false);
        await this.addCardToHand(this.dealerHand, 'bj-dealer-cards', true); // Carte face cachée
    ...
    async addCardToHand(hand, elementId, isHidden) {
        return new Promise(resolve => {
            setTimeout(() => {
                ...
                hand.push(card);
                const html = this.createCardHTML(card, isHidden);
                document.getElementById(elementId).insertAdjacentHTML('beforeend', html);
                AudioEngine.sfxDeal();
                resolve();
            }, 500); // Délai de 500ms par carte pour l'effet visuel
        });
    },
    ...
    async hit() {
        ...
        this.updateScores(false);
        if(this.getScore(this.playerHand) > 21) {
            this.endGame("SURCHARGE ! VOUS PERDEZ.", 0);
        } else if (this.getScore(this.playerHand) === 21) {
            this.stand(); // Auto-stand
        } else {
            document.getElementById('bj-play-controls').classList.remove('hidden');
        }
    },
    ...
    async stand() {
        ...
        // Retourne la carte cachée du dealer
        const dealerCardsHTML = document.getElementById('bj-dealer-cards').children;
        dealerCardsHTML[1].classList.add('revealed');
    ...
    endGame(msg, payout) {
        ...
        setTimeout(() => {
            ...
            if(this.bet > App.credits) this.bet = App.credits; // Auto-ajuste la mise max
            this.updateBetDisplay();
        }, 2000);
    },
...
// ==========================================
// 4. JEU : GRID RUNNER (Canvas 2D avec Lerp)
// ==========================================
const Runner = {
    canvas: null, ctx: null,
    animFrame: null, isPlaying: false,
    width: window.innerWidth, height: window.innerHeight,
    
    // Joueur
    player: { targetX: 0, x: 0, y: 0, size: 30 },
    lanes: [-200, 0, 200], // Positions X relatives au centre
    currentLane: 1,
    ...
    init() {
        ...
        this.canvas.addEventListener('touchstart', (e) => {
            e.preventDefault();
            this.touchControl(e.touches[0].clientX);
        }, {passive: false});
    },
    ...
    gameOver() {
        ...
        // Laisse tourner l'animation pour voir l'explosion
    ...
    createExplosion(x, y) {
        for(let i=0; i<30; i++) {
            this.particles.push({
                ...
                life: 1.0, color: Math.random()>0.5 ? '#00f3ff' : '#ff007f'
            });
        }
    },
    ...
    loop() {
        ...
        if(!this.isPlaying && this.particles.length === 0) {
            this.renderExplosionOnly = false;
        }
    },
    ...
    update() {
        if(this.isPlaying) {
            ...
            this.speed += 0.005; // Augmentation progressive
            ...
            // Mouvement fluide joueur (Lerp interpolation)
            this.player.x += (this.player.targetX - this.player.x) * 0.2;

            // Spawn Obstacles (Fréquence basée sur la vitesse)
            if(Math.random() < 0.03 + (this.speed * 0.001)) {
                let laneId = Math.floor(Math.random() * 3);
                // Prévention de murs impossibles
                let recentCount = this.obstacles.filter(o => o.y < 150).length;
                if(recentCount < 2) {
                    this.obstacles.push({ x: this.lanes[laneId], y: -100, width: 80, height: 20 });
                }
            }
        }
        ...
        for(let i = this.obstacles.length - 1; i >= 0; i--) {
            ...
            // Collision (AABB Box)
            if(this.isPlaying && 
               obs.y + obs.height > this.player.y - this.player.size && 
               obs.y < this.player.y + this.player.size) {
                
                // Tolérance de hit box (plus sympa pour le joueur)
                const pCenter = this.player.x;
                const oCenter = obs.x;
                if(Math.abs(pCenter - oCenter) < 40) {
                    this.gameOver();
                }
            }
    ...
    draw() {
        ...
        // Fond (Effet de traînée pour la vitesse)
        this.ctx.fillStyle = 'rgba(7, 7, 10, 0.4)';
        this.ctx.fillRect(0, 0, this.width, this.height);

        // Dessiner la grille/lignes de couloirs
        ...
        // Dessiner Obstacles (Neon Rouge)
        this.ctx.fillStyle = '#ff003c';
        this.ctx.shadowColor = '#ff003c';
        this.ctx.shadowBlur = 15;
        for(let obs of this.obstacles) {
            this.ctx.fillRect(cx + obs.x - obs.width/2, obs.y, obs.width, obs.height);
        }

        // Dessiner Joueur (Vaisseau Cyan) si vivant
        if(this.isPlaying) {
            this.ctx.fillStyle = '#00f3ff';
            this.ctx.shadowColor = '#00f3ff';
            this.ctx.beginPath();
            // Forme de flèche/vaisseau
